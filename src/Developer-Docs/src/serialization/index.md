...
I3M has built-in serializer that is used all over the place the engine and which is represented by a `Visit` trait. 
...
To use the macro, you must import all types related to `Visit` trait by `use i3m::core::visitor::prelude::*;`. Here's
an example:
